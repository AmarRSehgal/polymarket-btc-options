...
from fairvalue import Anchor, prob_sigma_c
from feed import BinanceBookTicker
...
DATA = Path(__file__).resolve().parent / "paper_data"
STEP_S = 0.2
MARKOUT_S = (5, 30, 60)
...
class MakerArm:
    def __init__(self, name: str, fv_kind: str, params: MakerParams):
        self.name, self.fv_kind, self.p = name, fv_kind, params
    ...
    def step(self, now: float, ctx: "Context"):
        ...
        fv = self.fair_value(now, book.mid_c, ctx.model_p)
        blocked = gate(now=now, window_ts=self.window_ts, strike_ok=ctx.strike > 0,
                       ...
                       binance_updated=ctx.bbo.updated, uses_binance=True, p=self.p)
        self.gates[blocked or "quoting"] += 1
        if blocked:
            self.venue.cancel_all(now)
            return
        bid, ask = desired_quotes(fv, ctx.sigma_c, self.position, book.best_bid, book.best_ask, self.p)
    ...
    def on_trade(self, tr: TapeTrade, ctx: "Context"):
        for f in self.venue.on_trade(tr):
            ...
            row = {"arm": self.name, "window_ts": self.window_ts, "t": f.t, "side": f.side,
                   ...
                   "btc": ctx.bbo.mid, "strike": ctx.strike, "model_p": ctx.model_p,
                   "markouts": {}}
...
class PaperAB:
    ...
    async def run(self):
        ...
        tasks = [asyncio.create_task(c) for c in (
            self.ctx.bbo.run(), self._windows(), self._decide(), self._settle(), self._status(),
